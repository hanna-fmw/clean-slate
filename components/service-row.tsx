...
function ServiceSummary({ service }: { service: Service }) {
  return (
    <div className="flex items-center gap-3 flex-1 min-w-0">
      <span className="font-mono font-bold text-sm shrink-0">{service.name}</span>
...
function ServiceDetail({ service }: { service: Service }) {
  return (
    <div className="space-y-4 text-sm">
      {service.url && (
        <div>
          <p className="text-xs text-[var(--muted)] mb-1 font-mono uppercase tracking-wider">URL</p>
          <a
            href={service.url}
            target="_blank"
      ...
      <div>
        <p className="text-xs text-[var(--muted)] mb-2 font-mono uppercase tracking-wider">Accounts</p>
